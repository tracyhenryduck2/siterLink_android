...
public class AlarmHistoryAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener, View.OnLongClickListener  {
    ...
    public AlarmHistoryAdapter(Context context, List<WarningHistoryBean> datas) {
        mContext=context;
        this.datas=datas;
        ...
        if(holder instanceof MyViewHolder){
            ...
            try {
                if(DeviceType.BATTERY.toString().equals(deviceBean.getModel())){
                    ((MyViewHolder) holder).contentView.setText(BatteryDescBean.getStatusShortString(datas.get(position).getWarningsubject()));
                }else{
                    ((MyViewHolder) holder).contentView.setText(BatteryDescBean.getStatusShortString(datas.get(position).getWarningsubject()));
                }
            ...
            ((AlarmHistoryAdapter.MyViewHolder) holder).itemView.setTag(datas.get(position));
